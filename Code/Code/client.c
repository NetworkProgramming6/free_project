...
int i=0;
int numberOfCards;
int clientCardNumber[4]={0,}; // 플레이어들 카드 개수
char playerStatus; //상태
int statusThreadNumber; //상태변화있는 쓰레드 넘버
struct card cardTable[4]; //출력할 테이블위 카드들
char playerName[4][30]; //플레이어들 이름
...
void *readSrv(void * parm) //계속 읽기 쓰레드
{
    ...
    int readLen;
    while(1){ 
        recv(clientSocketDescriptor,&playerStatus,sizeof(char),0); //서버로부터 계속 정보 받아옴
        ...
        else if(playerStatus=='o') //플레이어가 벨을 눌렀음, 벨누를때맞음
        {
            printNullCard(); 
        ...
        else if(playerStatus=='x') //플레이어가 벨을 잘못눌렀음
        {
...
int main(int argc, char** argv)
{
    ...
    clientSocketDescriptor=socket(AF_INET, SOCK_STREAM,0);//클라이언트 소켓 선언함
