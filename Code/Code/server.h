...
extern int num; //접속인원
extern int clntCardNum[4]; //플레이어 카드 개수
extern int tableCardNum[4]; //테이블 카드 개수
extern pthread_t thread[4]; //플레이어 4명 쓰레드
extern int status_num; //몇번 플레이어 상태가 변했는지
extern char status; //현재 상태 r,y,g,p:빨강,노랑,초록,보라색 카드 뒤집힘 /  o:벨 맞음 / x:벨 잘못누름 / e:게임종료
...
void bell_O(int threadNum)
{
	...
	{
		if(i==threadNum)
			clntCardNum[threadNum]+=sum; //벨 누른 사람 카드 개수 증가
		//else
...
void bell_X(int threadNum)
{
	...
	for(i=0;i<4;i++)
	{
		...
		{
			clntCards[i][clntCardNum[i]+tableCardNum[i]]=temp[j]; //카드 하나 받음
			j++;
			clntCardNum[i]++; //카드 한개 획득
		}
		else //벨 잘못 누른 플레이어
		{
			clntCardNum[i]-=3; //카드 3개 손실
		}
	}
}
...
{
	for(int i=0;i<4;i++)
		if(clntCardNum[i]<=0) //플레이어 카드 0이하인지 확인
		{
			status='e';
			for(int j=0;j<4;j++)
				statusCheck[j]=1;
