...
int statusThreadNumber=-1; //누구 상태가 변했는지
char playerStatus='\0'; //현재 상태 r,y,g,p:빨강,노랑,초록,보라색 카드 뒤집힘 /  o:벨 맞음 / x:벨 잘못누름 / e:게임종료
...
void * client_module(void * data)
{
    ...
    for(int i=0;i<4;i++) //플레이어들 이름 알려주기
    {
        int tempSize=0;
    ...
    while(1)
    {
        ...
        else if(strcmp(rBuff,"b")==0)// && playerStatus!='o' && playerStatus!='x')
        { //bell을 둘이서 눌렀을때 처리하는 부분
            if(bell()==1 && playerStatus!='o') //벨 누를 상황인데, 그턴에 처음누른사람
            {
                bell_O(threadNum);
                playerStatus='o';
                statusThreadNumber=threadNum;
                checkGameEnd(); //게임 끝나는지 확인
            }
